...
\subsection{Aplicação do modelo climático}
...
As correlações entre o ONI e a anomalia de PSN são negativas em todas as defasagens e em todos os biomas, de modo que o El Niño reduz e a La Niña eleva a PSN, com magnitude entre 0,03 e 0,19 (Figura~\ref{fig:defasagem}). A defasagem de maior correlação difere entre os biomas: a Caatinga responde em fase, com o máximo em $L = 0$ ($\rho = -0{,}19$); a Mata Atlântica, com dois meses de atraso ($\rho = -0{,}16$); e o Cerrado, com dez meses ($\rho = -0{,}16$), permanecendo a correlação praticamente constante entre $L = 1$ e $L = 12$. A resposta do bioma semiárido é, portanto, imediata e de curta duração, e a do Cerrado, persistente. O contraste é compatível com a dinâmica de pulsos de recursos dos ecossistemas semiáridos \citep{schwinning2004} e com a integração da água no solo ao longo de semanas nos sistemas de savana \citep{oliveira2005}. Mesmo nos máximos, o coeficiente ao quadrado não ultrapassa 3,6\% da variância das anomalias.
...
\subsection{Resposta reproduzida pelo modelo}
...
\midrule
Mata Atlântica & TST & El Niño & $+3{,}07$  & $+1{,}7$ a $+4{,}4$  & $<$0,001 (0,002) & $\mathbf{+0{,}9}$ \textbf{a} $\mathbf{+5{,}3}$ & \textbf{0,005} (0,15) \\
Caatinga       & PSN & La Niña & $+10{,}45$ & $+4{,}4$ a $+16{,}4$ & 0,001 (0,005) & $\mathbf{+0{,}4}$ \textbf{a} $\mathbf{+16{,}9}$ & \textbf{0,044} (0,44) \\
Mata Atlântica & PSN & El Niño & $-5{,}44$  & $-8{,}7$ a $-2{,}3$  & 0,001 (0,005) & $\mathbf{-9{,}2}$ \textbf{a} $\mathbf{-0{,}05}$ & \textbf{0,048} (0,44) \\
\addlinespace
...
Os valores de $p$ citados acima são brutos. Submetidos à correção de Benjamini-Hochberg sobre a família dos 30 testes, 9 dos 10 efeitos sobrevivem no esquema convencional, mas nenhum dos três sobrevive no esquema por episódio: os valores de $q$ são 0,15 para a temperatura da Mata Atlântica e 0,44 para as duas respostas de PSN. O mesmo ocorre quando a correção é restrita aos seis testes da PSN ($q = 0{,}14$ tanto na Mata Atlântica em El Niño quanto na Caatinga em La Niña). Com 24 anos de registro, portanto, nenhum efeito isolado do ENSO sobre a produtividade destes biomas se sustenta sob inferência por episódio com controle da taxa de falsas descobertas.

Os três efeitos de menor $p$ são justamente os dois polos do gradiente: o aquecimento da superfície da Mata Atlântica em El Niño (IC95\% $+0{,}9$ a $+5{,}3$~p.p.; $p = 0{,}005$), o mais robusto do conjunto, a redução da PSN desse mesmo bioma e fase (IC95\% $-9{,}2$ a $-0{,}05$~p.p.; $p = 0{,}048$) e o aumento da PSN da Caatinga em La Niña (IC95\% $+0{,}4$ a $+16{,}9$~p.p.; $p = 0{,}044$). Os dois últimos são marginais mesmo antes da correção: seus intervalos quase tocam zero. No Cerrado, o aumento de 11,7~p.p.\ da PSN em La Niña passa a $p = 0{,}081$, com intervalo de $-1{,}5$ a $+24{,}3$~p.p., quando as nove sequências de La Niña são tomadas como as nove observações que de fato são.

A assimetria descrita nas Seções~3.2 e 3.3, com a Mata Atlântica associada à fase quente e os biomas sazonais à fase fria, é, portanto, consistente entre os dois esquemas, e nenhum resultado troca de sinal. O que a verificação mostra é que a magnitude de cada efeito é estimada com precisão limitada pelo número de episódios disponíveis, e não pelo número de meses, e que essa precisão não basta para sustentar qualquer efeito isolado sob controle de comparações múltiplas.
...
Quando a unidade amostral passa do mês para o episódio, o padrão assimétrico se mantém e nenhum resultado troca de sinal, mas os intervalos de confiança alargam-se 1,67 vez em mediana e apenas três efeitos mantêm $p$ abaixo de 0,05, entre os quais os dois polos do gradiente. Nenhum deles sobrevive à correção para comparações múltiplas. Os resultados caracterizam, portanto, três regimes distintos de associação entre o ENSO e a produtividade, e indicam que sua magnitude deve ser reportada com a precisão que o número de episódios, e não o de meses, permite. Estabelecer qualquer um desses efeitos isoladamente exigirá séries mais longas ou maior replicação espacial.
